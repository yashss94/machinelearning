...
def EM(tau1, tau2, mew1, mew2, S1, S2, X):
    ...
    while iters < 100:
        ...
        for i in range(len(X)):
            temp_S1 = np.add(temp_S1, np.dot(np.dot(p_a[i], np.atleast_2d(np.subtract(X[i], mew1)).T),
                                             np.atleast_2d(np.subtract(X[i], mew1))))
            temp_S2 = np.add(temp_S2, np.dot(np.dot(p_b[i], np.atleast_2d(np.subtract(X[i], mew2)).T),
                                             np.atleast_2d(np.subtract(X[i], mew2))))
